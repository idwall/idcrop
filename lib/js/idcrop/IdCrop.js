...
class IdCrop {
  constructor(config) {
    this.config = {
      numPoints: config.numPoints ? config.numPoints : 4,
      containers: {
  ...
  reset() {
    ...
    if (this.config.containers.previewArea)
      this.config.containers.previewArea.style.backgroundImage = "";
  ...
  clear(self) {
    if (
      ...
      self.cropArea.isDrawing == false &&
      (self.handles.length == self.config.numPoints || self.config.numPoints == Infinity)
    ) {
      if (this.config.containers.previewArea) self.drawPreview(self);
