...
class CropArea {
  ...
  crop(preview, points) {
    ...
    const context = auxCanvas.getContext("2d");
    ...
    for (const point of points) {
      realPoints.push(new Point(point.x * this.img.ratio,
                                point.y * this.img.ratio));
    }

    const renderer = new Polygon(preview, auxCanvas);
    renderer.drawWithOverlay(realPoints, 0, false, this.img.img);
