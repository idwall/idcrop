const IdCrop = (function() {
    ...
    const init = function(container_id, display_id, preview_id) {
    ...
    const sizeOverlays = function() {
        ...
        overlays[3].style.width =
            crop_offset.offsetWidth -
            (crop.offsetLeft + crop.offsetWidth) +
            "px";
    };
    ...
    const startResizing = function(event) {
        ...
        const crop = elements.crop,
            ...
            crop_border = parseInt(
                getComputedStyle(crop, null).getPropertyValue(
                    "border-left-width"
                ),
                10
            );
        ...
        document.onmousemove = function(event) {
            ...
            let rel_max_width =
                    max_width -
                    (crop_bounds.left - container_bounds.left - bg_image.left),
                rel_max_height =
                    max_height -
                    (crop_bounds.top - container_bounds.top - bg_image.top);

            let e_width = clip(
                    crop_bounds.width - delta_x,
                    min_width,
                    rel_max_width
                ),
                w_width = clip(
