const IdCrop = (function() {
    ...
    const startResizing = function (event) {
        ...
        document.onmousemove = function(event) {
            ...
            // The getBoundingClientRect() function doesn't count the border
            // of the crop area. So we remove it manually. 
            let left = (crop_bounds.left - container_bounds.left) - delta_x - (crop_border * 2),
                top = (crop_bounds.top - container_bounds.top) - delta_y - (crop_border * 2);
 
            if (direction == "idwall-resize-e") {
                crop.style.width = crop_bounds.width - delta_x + "px";
            }
            if (direction == "idwall-resize-s") {
                crop.style.height = crop_bounds.height - delta_y + "px";
            }
            ...
            if (direction == "idwall-resize-se") {
                crop.style.height = crop_bounds.height - delta_y + "px"; // s
                crop.style.width = crop_bounds.width - delta_x + "px";  //e              
            }
            if (direction == "idwall-resize-sw") {
                crop.style.height = crop_bounds.height - delta_y + "px"; //s
                crop.style.width = crop_bounds.width + delta_x + "px"; // w
                crop.style.left = left + "px";                
            }
            if (direction == "idwall-resize-ne") {
                crop.style.height = crop_bounds.height + delta_y + "px"; // n
                crop.style.top = top + "px";
                crop.style.width = crop_bounds.width - delta_x + "px"; // e               
            }
