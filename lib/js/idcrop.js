const IdCrop = (function() {
    ...
    const startResizing = function (event) {
        ...
        const crop = elements.crop,
              ...
              crop_border = parseInt(getComputedStyle(crop, null)
                .getPropertyValue("border-left-width"), 10);

        const e_boundary = bg_image.left,
              w_boundary = bg_image.left + bg_image.width,
        ...
        document.onmousemove = function(event) {
            ...
            let left = clip((crop_bounds.left - container_bounds.left) -
                        delta_x - (crop_border * 2), e_boundary, w_boundary),
                top = clip((crop_bounds.top - container_bounds.top) -
                        delta_y - (crop_border * 2), n_boundary, s_boundary);

            // The maximum width and height are relative
            // to the position of the crop area.
            let rel_max_width = max_width - (crop_bounds.left -
                                container_bounds.left - bg_image.left),
                rel_max_height = max_height - (crop_bounds.top -
                                 container_bounds.top - bg_image.top);
            ...
            let e_width = clip(crop_bounds.width - delta_x, min_width, rel_max_width),
                ...
                s_height = clip(crop_bounds.height - delta_y, min_height, rel_max_height),
    ...
    const previewCropped = function() {
        ...
        image.onload = function() {
            ...
            let relTop = crop_bounds.top - container_bounds.top;
            let relLeft = crop_bounds.left - container_bounds.left;
